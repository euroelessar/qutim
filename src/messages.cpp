...
void MessagesHandler::handleSNAC(AbstractConnection *conn, const SNAC &sn)
{
	...
	switch((sn.family() << 16) | sn.subtype())
	{
		...
		case MessageFamily << 16 | MessageSrvAck: {
			...
			debug() << QString("Server accepted message for delivery to %1 on channel %2").
					arg(uin).arg(channel);
			break;
		}
	...
	if(tlvs.contains(0x0005))
	{
		...
		else
		{
			...
			if(!plugins.isEmpty())
			{
				...
				debug() << IMPLEMENT_ME << QString("Message (channel 2) from %1 with type %2 and guid %3 is not processed.").
	...
	if(guid == ICQ_CAPABILITY_PSIG_MESSAGE)
	{
		...
		else if(MsgPlugin)
		{
			...
			if(pluginType == MSG_XSTRAZ_SCRIPT)
			{
				...
				debug() << "Unhandled plugin message"
