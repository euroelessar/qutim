...
struct XStatus
{
	...
	XStatus(const QString &status_, const QString &icon_,  qint8 mood_, const Capability &capability_):
		status(status_), capability(capability_), mood(mood_)
	{
		icon = QString("user-xstatus-") + icon_;
	}
	QString status;
	QString icon;
	qint8 mood;
...
bool Xtraz::handelXStatusCapabilities(IcqContact *contact, const Capabilities &caps, qint8 mood)
{
	foreach(const XStatus &status, data()->xstatuses)
	{
		...
		{
			contact->setProperty("xstatusIcon", status.icon);
...
void Xtraz::parseAwayMsg(IcqContact *contact, QXmlStreamReader &xml)
{
	...
	if(uin == contact->id())
	{
		...
		contact->setProperty("xstatusIcon", data()->xstatuses[index].icon);
