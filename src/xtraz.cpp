...
class XtrazPrivate: public QObject
{
public:
	XtrazPrivate();
...
bool Xtraz::handelXStatusCapabilities(IcqContact *contact, const Capabilities &caps, qint8 mood)
{
	foreach(const XStatus &status, data()->xstatuses)
	{
		if(caps.match(status.capability) || status.mood == mood)
		{
			...
			contact->setProperty("statusTitle", status.status);
			return true;
		}
	}
	return false;
...
void Xtraz::parseSrv(IcqContact *contact, QXmlStreamReader &xml, bool response)
{
	while(!xml.atEnd())
	{
		xml.readNext();
...
void Xtraz::parseAwayMsg(IcqContact *contact, QXmlStreamReader &xml)
{
	...
	if(uin == contact->id())
	{
		...
		contact->setProperty("statusIcon", data()->xstatuses[index].icon);
		contact->setProperty("statusText", desc);
		qDebug() << "xstatus" << index << data()->xstatuses[index].status;
	}
}

void Xtraz::parseRequest(IcqContact *contact, QXmlStreamReader &xml)
{
	QString reqId;
	QString uin;
	while(!xml.atEnd())
