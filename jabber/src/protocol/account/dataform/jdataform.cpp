	...
	: QWidget(parent), d_ptr(new JDataFormPrivate)
{
	...
	setLayout(layout);
	layout->addWidget(AbstractDataForm::get(root));
//	for (int i = 0; i < form->fieldsCount(); i++) {
//		jreen::DataFormField::Ptr field = form->field(i);
...
//		elem->field = field.data();
//		if (elem->import_value)
//			(*elem->import_value)(elem->obj, field.data());
...
//		int column = 0;
//		(*elem->add_widget)(layout, elem->obj, field->description(), row, column);
//	}
}
