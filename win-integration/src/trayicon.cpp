...
public:
	...
	bool eventFilter(QObject *o, QEvent *e)
	{
		if (QEvent::ActivationChange == e->type()) {
			if (!ServiceManager::getByName("ContactList")->property("widget").value<QWidget*>()->isActiveWindow())
				tray->clActivationStateChanged();
		}
		return false;
	}
};

NotSimpleTray::NotSimpleTray()
{
	...
	ServiceManager::getByName("ContactList")->property("widget").value<QWidget*>()->installEventFilter(new ClEventFilter(this));
	m_icon->setContextMenu(menu());
	qApp->setQuitOnLastWindowClosed(false);
	activationStateChangedTime = QDateTime::currentMSecsSinceEpoch();
}

void NotSimpleTray::clActivationStateChanged()
{
	activationStateChangedTime = QDateTime::currentMSecsSinceEpoch();
}

void NotSimpleTray::onActivated(QSystemTrayIcon::ActivationReason reason)
{
	if (reason == QSystemTrayIcon::Trigger) {
		if (m_sessions.isEmpty()) {
			if (QObject *obj = ServiceManager::getByName("ContactList")) {
				if (QDateTime::currentMSecsSinceEpoch() - activationStateChangedTime < 200)
...
void NotSimpleTray::onSessionCreated(qutim_sdk_0_3::ChatSession *session)
{
...
void NotSimpleTray::onSessionDestroyed()
{
	onUnreadChanged(MessageList());
}

void NotSimpleTray::onUnreadChanged(qutim_sdk_0_3::MessageList unread)
{
...
void NotSimpleTray::timerEvent(QTimerEvent *timer)
{
...
void NotSimpleTray::onAccountDestroyed(QObject *obj)
{
...
void NotSimpleTray::onAccountCreated(qutim_sdk_0_3::Account *account)
{
...
void NotSimpleTray::onStatusChanged(const qutim_sdk_0_3::Status &status)
{
...
void NotSimpleTray::validateProtocolActions()
{
