...
void MetaInfo::handleShortInfo(QObject *reqObject, const DataUnit &data)
{
	...
	QString gender = genders()->value(data.read<qint8>());
...
void MetaInfo::handleBasicInfo(QObject *reqObject, const DataUnit &data)
{
	...
	QString home_country = countries()->value(data.read<quint16>(LittleEndian));
	...
	debug(Verbose) << "Basic info" << nick << first_name << last_name << email
			...
			<< cell_phone << home_zip_code << home_country << GMT << auth
...
void MetaInfo::handleMoreInfo(QObject *reqObject, const DataUnit &data)
{
	...
	QString language1 = languages()->value(data.read<quint8>());
	QString language2 = languages()->value(data.read<quint8>());
	QString language3 = languages()->value(data.read<quint8>());
	data.skipData(2); // 0x0000 unknown
	QString original_city = readString(data);
	QString original_state = readString(data);
	QString original_country = countries()->value(data.read<quint16>(LittleEndian));
	...
	debug(Verbose) << "More info" << age << gender << homepage << birth_year
			...
			<< original_city << original_state << original_country << time_zone;
}
...
void MetaInfo::handleWork(QObject *reqObject, const DataUnit &data)
{
	...
	QString work_country = countries()->value(data.read<quint16>(LittleEndian));
	QString work_company = readString(data);
	QString work_department = readString(data);
	QString work_position = readString(data);
	QString work_ocupation = occupations()->value(data.read<quint16>(LittleEndian));
	QString work_webpage = readString(data);

	debug(Verbose) << "Work info" << work_city << work_state << work_phone << work_fax
			<< work_address << work_zip << work_country << work_company << work_department
			<< work_position << work_ocupation << work_webpage;
}
...
{
	QList<Category> interestsList = handleCatagories(data, *interests());
	debug(Verbose) << "Interest" << interestsList;
}

void MetaInfo::handleAffilations(QObject *reqObject, const DataUnit &data)
{
	QList<Category> pastsList = handleCatagories(data, *pasts());
	debug(Verbose) << "Pasts" << pastsList;

	QList<Category> affilationsList = handleCatagories(data, *affilations());
	debug(Verbose) << "Affilations" << affilationsList;
}

QList<MetaInfo::Category> MetaInfo::handleCatagories(const DataUnit &data, const FieldNamesList &list)
{
