...
bool NowPlaying::load(){
	...
	eventTypes.icqChangeXstatus = Event::registerType("icq-change-xstatus");
    return true;
}
...
void NowPlaying::setOscarStatus(const QString& acc, const QString& message){
	...
	extStatus.insert("name", "music");
	extStatus.insert("desc", message);

	m_current_status_name.insert(acc, "music");
	Event ev(eventTypes.icqChangeXstatus);
	ev.args[0] = extStatus;
    ...
    if (m_global_settings.is_working){
        ...
        if (proto_id == "icq"){
            ...
            if (!config.deactivated){
				QVariantHash xstatus = m_oscar_protocol->account(acc)->status().extendedInfo("xstatus");
                if (config.sets_current_status &&
					xstatus.value("description") != getMessage(acc, m_player->trackInfo(), empty)){
                    return true;
